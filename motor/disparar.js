...
/* ============================================================
   ...
   Uso:
     ...
     node motor/disparar.js couve-e-flor estancia-caipira ...   (só esses)
...
function main(){
  ...
  for(const slug of dirs){
    if(filtro.length && filtro.indexOf(slug) === -1) continue;
    let data;
    ...
    lista.push({
      ...
      cidade: cid,
      url: url,
      msg: buildPitch(name, onde, url)
    });
  }

  const html = render(lista);
  const out = path.join(ROOT, 'disparo.html');
  fs.writeFileSync(out, html, 'utf8');

  console.log('✓ Fila de disparo gerada: ' + lista.length + ' contato(s) com WhatsApp');
  lista.forEach(function(c){ console.log('  • ' + c.name + '  (' + c.phone + ')'); });
...
+ '.card.done{opacity:.45}.card h3{font-size:1.1rem}.meta{color:#8696a0;font-size:.84rem;margin:2px 0 12px}'
+ '.meta a{color:#53bdeb;text-decoration:none}textarea{width:100%;background:#0b141a;color:#e9edef;border:1px solid #2a3942;border-radius:8px;padding:12px;font-family:inherit;font-size:.92rem;min-height:120px;resize:vertical}'
...
+ '<p class="sub">Deixe o <b>WhatsApp Web logado</b>. Clique em <b>Abrir no WhatsApp</b> → a conversa abre com a mensagem pronta → aperte <b>Enter</b>. O envio é seu (mantém seu número seguro).</p>'
...
+ '<div id="list"></div></div>'
+ '<script>var DADOS=' + dados + ';'
+ 'var KEY="disparo-enviados";'
+ 'function getSent(){try{return JSON.parse(localStorage.getItem(KEY)||"{}")}catch(e){return {}}}'
+ 'function setSent(s){localStorage.setItem(KEY,JSON.stringify(s))}'
+ 'function esc(s){return (s==null?"":String(s)).replace(/[&<>"]/g,function(c){return {"&":"&amp;","<":"&lt;",">":"&gt;","\\"":"&quot;"}[c]})}'
+ 'function prog(){var s=getSent();var n=DADOS.filter(function(c){return s[c.slug]}).length;document.getElementById("prog").textContent="Enviados: "+n+" de "+DADOS.length}'
+ 'function waLink(c){return "https://wa.me/"+c.wa+"?text="+encodeURIComponent(document.getElementById("msg-"+c.slug).value)}'
+ 'function render(){var s=getSent();var h=DADOS.map(function(c){return ('
+ '"<div class=\\"card"+(s[c.slug]?" done":"")+"\\" id=\\"card-"+c.slug+"\\">"+'
+ '"<h3>"+esc(c.name)+" <span class=\\"tag"+(s[c.slug]?" ok":"")+"\\" id=\\"tag-"+c.slug+"\\">"+(s[c.slug]?"✓ enviado":"pendente")+"</span></h3>"+'
+ '"<div class=\\"meta\\">"+esc(c.phone)+(c.cidade?" · "+esc(c.cidade):"")+" · <a href=\\""+esc(c.url)+"\\" target=\\"_blank\\">ver site</a></div>"+'
+ '"<textarea id=\\"msg-"+c.slug+"\\">"+esc(c.msg)+"</textarea>"+'
+ '"<div class=\\"row\\">"+'
+ '"<a class=\\"send\\" id=\\"send-"+c.slug+"\\" target=\\"_blank\\" rel=\\"noopener\\">Abrir no WhatsApp →</a>"+'
+ '"<button class=\\"mark\\" data-s=\\""+c.slug+"\\">Marcar enviado / desfazer</button>"+'
+ '"</div></div>")}).join("");'
+ 'document.getElementById("list").innerHTML=h;'
+ 'DADOS.forEach(function(c){var a=document.getElementById("send-"+c.slug);var ta=document.getElementById("msg-"+c.slug);function sync(){a.href=waLink(c)}sync();ta.addEventListener("input",sync);a.addEventListener("click",function(){var s=getSent();s[c.slug]=true;setSent(s);document.getElementById("card-"+c.slug).classList.add("done");var t=document.getElementById("tag-"+c.slug);t.textContent="✓ enviado";t.classList.add("ok");prog()})});'
+ 'document.querySelectorAll(".mark").forEach(function(b){b.addEventListener("click",function(){var sl=b.getAttribute("data-s");var s=getSent();if(s[sl]){delete s[sl]}else{s[sl]=true}setSent(s);render()})});'
+ 'prog()}'
+ 'document.getElementById("reset").addEventListener("click",function(){if(confirm("Limpar todas as marcações de enviado?")){setSent({});render()}});'
+ 'render();'
+ '</script></body></html>';
}
